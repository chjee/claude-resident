...
`<name> [start|stop|restart|graceful-restart|status|check|health|shutdown|cleanup-memory|logs|attach]` 형식으로 인스턴스별 관리. 진입점은 `claude-resident`, 명령별 helper module은 `lib/claude-resident/`에 둔다.

- `restart`: tmux 세션을 즉시 kill 후 재시작 (daily 저장 없음)
- `graceful-restart`: `__SHUTDOWN__` 전송 → daily 저장 대기 → 재시작. 운영 세션을 안전하게 교체할 때 사용
...
`claude-resident-health@.timer`가 5분마다 `claude-resident <name> health`를 실행해 tmux 세션 생존 여부를 확인하고, 소멸 시 자동 복구한다.

- **복구 범위**: 프로세스/세션 소멸 + Telegram poller 소멸. Claude hang이나 plugin 응답 불능은 감지하지 못함
- **poller 감지**: tmux 세션이 살아 있어도 `bot.pid`가 죽어 있으면 세션을 재시작함
- **의도적 정지 보호**: `systemctl --user is-active` gate로 `systemctl stop` 후에는 복구하지 않음
- **상태 기록**: 마지막 health check의 reason/시각/재시작 여부와 `poller_alive`는 `~/.local/state/claude-resident/<name>/health-state.json`에 기록
...
- **주의**: Telegram poller를 다른 Claude 세션이 가져간 경우는 `bot.pid` 생존만으로는 구분 불가. `claude-resident <name> check`의 global token warning과 `TELEGRAM_STATE_DIR` 분리 상태를 확인한다.
...
`claude-resident-health@.timer`는 5분마다 `claude-resident $NAME health`를 실행해 tmux 세션 생존 여부를 확인하고, main service가 active인데 세션이 없으면 재시작한다. 마지막 health check 결과는 `~/.local/state/claude-resident/$NAME/health-state.json`에 남는다.
...
- 마지막 health state 요약 (`action`, `reason`, `checked_at`, 재시작 전후 세션 존재 여부, `poller_alive`)
- Telegram poller pid 생존 여부 (`TELEGRAM_STATE_DIR/bot.pid`)
- tmux 세션 실행 여부
...
(레포 루트)
    ...
    ~/.local/state/claude-resident/<name>/health-state.json
```
...
> `agent-events.jsonl`은 세션 시작/종료/에러 등 주요 이벤트를 ndjson으로 기록한다. 각 줄은 `{"ts":..., "level":..., "instance":..., "event":..., "msg":...}` 형식이며, `jq` 등으로 파싱하거나 외부 모니터링 시스템에 연동할 수 있다.
> `health-state.json`은 health timer가 마지막으로 어떤 판단을 했는지 기록한다. 예: `noop/session_present`, `restart/session_missing`, `restart/poller_dead`, `skip_inactive/service_inactive`.
