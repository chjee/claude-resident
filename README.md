...
- **상태 기록**: 마지막 health check의 reason/시각/재시작 여부는 `~/.local/state/claude-resident/<name>/health-state.json`에 기록
...
> `health-state.json`은 health timer가 마지막으로 어떤 판단을 했는지 기록한다. 예: `noop/session_present`, `restart/session_missing`, `skip_inactive/service_inactive`.
